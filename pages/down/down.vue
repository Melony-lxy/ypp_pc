		...
		<view class="bgimg-code">
			...
			<view class="code" v-if="inviteCode">
				...
				<view class="codeInput">
					...
					<view @click="oncopy(inviteCode	)">复制</view>
				</view>
			</view>
			<view v-else style="position: absolute;top: 50%;width: 100%; transform: translateY(-50%);">
				<image @click="goDownApp()" :src="globalstore.upImageuler('/static/down/button.png')" mode="widthFix"></image>
			</view>
		</view>
...
<script setup>
	...
	function getHeight() {
		const elemet = uni.getWindowInfo()
		height.value = elemet.safeArea.height
	}
	...
	onLoad((e)=>{
		if(e.inviteCode) inviteCode.value = e.inviteCode
	})
