<template>
	<view class="content">
			...
			<view class="body-left">
				<view class="left-shop" v-if="marchantInfo">
					<view class="shop-left">
						<view class="shopImg">
							<image
								:src="marchantInfo.userAvatar ? marchantInfo.userAvatar : globalstore.upImageuler('/static/profile.png')"
								mode="aspectFill" style="width: 88rpx;height: 88rpx;"></image>
						</view>
						<view class="shopInfo">
							<view>{{marchantInfo.nickName}}</view>
							<view class="infoImg">
								<image
									:src="marchantInfo.businessType ? globalstore.upImageuler('/static/slices/march.png') : globalstore.upImageuler('/static/slices/my.png')"
									mode="heightFix" style="height: 100%;"></image>
							</view>
						</view>
					</view>
				...
				<view v-if="orderMarch" class="order">
					<view class="left-shop" style="border-radius: 20rpx 20rpx 0 0;margin: 0;">
						<view class="shop-left">
							<view class="shopImg">
								<image
									:src="orderMarch.sellerAvatar ? orderMarch.sellerAvatar : globalstore.upImageuler('/static/profile.png') "
									mode="aspectFill" style="width: 88rpx;height: 88rpx;"></image>
							</view>
							<view class="shopInfo">
								...
								<view class="infoImg">
									<image
										:src=" orderMarch.sellerBusinessType ? globalstore.upImageuler('/static/slices/march.png') : globalstore.upImageuler('/static/slices/my.png')"
										mode="heightFix" style="height: 100%;"></image>
								</view>
							</view>
						</view>
					...
					<view class="left-shop" style="border-radius: 0 0 20rpx 20rpx;margin: 0;">
						<view class="shop-left">
							<view class="shopImg">
								<image
									:src="orderMarch.buyerAvatar ? orderMarch.buyerAvatar : globalstore.upImageuler('/static/profile.png') "
									mode="aspectFill" style="width: 88rpx;height: 88rpx;"></image>
							</view>
							<view class="shopInfo">
								<view>{{orderMarch.buyerNickName}}<text class="march" style="background-color: #00b0f9;">买家</text>
								</view>
								<view class="infoImg">
									<image
										:src="orderMarch.buyerBusinessType ? globalstore.upImageuler('/static/slices/march.png') : globalstore.upImageuler('/static/slices/my.png')"
										mode="heightFix" style="height: 100%;"></image>
								</view>
							</view>
						</view>
					...
					<view class="content-info">
						<view class="body-right-top">
							...
							<view class="success" v-if="goodsInfo.orderStatus == 3 && !orderCode">
								<image :src="globalstore.upImageuler('/static/slices/salesOver.png')" mode="widthFix"></image>
							</view>
							<view class="rightName" style="display: flex;align-items: center;">
								<view class="nameTitle" style="align-items: center;">{{goodsInfo.productName}}
									<image v-if="goodsInfo.currentCargoStatus" style="width: 56rpx;height: 30rpx;"
										:src="globalstore.upImageuler('/static/slices/presale.png')" mode="aspectFit"></image>
									<text v-else
										style="color: #fff;background-color: #FF5500;font-size: 22rpx;padding: 0 10rpx;border-radius: 6rpx;">售</text>
								</view>
							...
							<view class="currenShop" v-if="goodsInfo.currentCargoStatus">
								<view class="presale">
									<view class="presale-price">
										<view><span>¥</span>{{goodsInfo.totalSellingPrice}}</view>
										<view class="unit">
											<view>
												单价:¥{{goodsInfo.transactionUnitPrice ||  (goodsInfo.totalSellingPrice/goodsInfo.souvenirCoinNumber).toFixed(2) || 0 }}
											</view>
											<view>
												数量:{{goodsInfo.souvenirCoinNumber || goodsInfo.identifyQuantity}}{{goodsInfo.unit ? goodsInfo.unit : '张'}}
											</view>
										</view>
									</view>
									<view class="presale-time">
										...
										<view>
											{{goodsInfo.issuanceDate ? `预计${getAfterDays(goodsInfo.issuanceDate,goodsInfo.followReleaseDays)}点前发货` : `预计官方发行后${goodsInfo.followReleaseDays}天内发货` }}
										</view>
									</view>
								...
								<view class="presale-process">
									...
									<view class="process-list">
										...
										<view class="item">
											...
											<view>发货日期48小时前支付货款</view>
										</view>
									</view>
								</view>
							</view>
							<view class="rightPicker" v-if="!orderCode && !goodsInfo.currentCargoStatus">
								<view class="picker">
									¥{{orderCode ? orderMarch.totalPrice : goodsInfo.totalSellingPrice}}
								</view>
								<view class="margin">
									单价:¥{{goodsInfo.transactionUnitPrice ||  (goodsInfo.totalSellingPrice/goodsInfo.souvenirCoinNumber).toFixed(2) || 0 }}
								</view>
							...
							<view class="rightPicker" v-else>
								<view class="picker" v-if="orderMarch">
									¥{{orderMarch.totalPrice}}
								</view>
								<view class="margin" v-if="orderMarch">
									单价:¥{{orderMarch.totalPrice/orderMarch.totalCount.toFixed(2) || 0}}
								</view>
								<view class="margin" v-if="orderMarch">
									数量:{{orderMarch.totalCount}}
								</view>
							</view>
							<view class="rightInfo" id="infodom">
								<view class="infoItem">
									...
									<view class="infoRight">
										{{goodsInfo.validityPeriodBeginTime}}-{{goodsInfo.validityPeriodEndTime}}
									</view>
								</view>
								<view class="infoItem">
									<view class="infoLift">最小确认数量</view>
									<view class="infoRight">
										{{pageType == 1 ? goodsInfo.minimumConfirmationQuantity || '1' : goodsInfo.purchaseOrderItemVOS[0].minimumConfirmationQuantity || '1'}}{{pageType == 1 ? (goodsInfo.unit ? goodsInfo.unit : '张') : (goodsInfo.purchaseOrderItemVOS[0].unit ? goodsInfo.purchaseOrderItemVOS[0].unit : '张')}}
									</view>
								</view>
								<view class="infoItem">
								...
								<view class="infoItem">
									...
									<view class="infoRight">
										<view class="good">
											{{pageType == 1 ? productCondition[goodsInfo.productCondition] : productCondition[goodsInfo.purchaseOrderItemVOS[0].productCondition] }}
										</view>
									</view>
								</view>
								<view class="infoItem">
									...
									<view class="infoRight-bont">
										<view class="bond">
											{{goodsInfo.needTradeMargin ? `${goodsInfo.tradingMarginRatio * 100}%` : '无'}}
										</view>
										<view class="bondtext" v-if="goodsInfo.needTradeMargin">
											*此商品采用保证金交易:为保护买卖双方的利益，双方都需要缴纳保证金，如有违约行为，违约方将被扣除保证金</view>
									</view>
								</view>
								<view class="infoItem" style="align-items: flex-start;">
						...
						<view class="details">
							...
							<view v-if="goodsInfo.descriptionDetails || goodsInfo.purchaseOrderItemVOS" class="details-title">
								{{pageType == 1 ? goodsInfo.descriptionDetails : goodsInfo.purchaseOrderItemVOS[0].descriptionDetails}}
							</view>
							<view v-else style="color: #999; padding: 12rpx 0 30rpx;">暂无数据</view>
							<view v-if="pageType==1">
								<view v-if="goodsInfo.orderType == 0" class="details-num">
									<view class="details-info">打包出售包含冠号</view>
									<view v-for="(item,index) in goodsInfo.itemDTOList" :key="item.id">
										<GoodsNotes :data="item" :isShowSafePrice="true" :productName="goodsInfo.productName"
											:appraisalInstitutionId="goodsInfo.appraisalInstitutionId"
											:productCategoryId="goodsInfo.productCategoryId"></GoodsNotes>
									</view>
								</view>

								<GoodsTree v-else :pageType="pageType"
									:labelInfos='goodsInfo.labelInfos ? JSON.parse(goodsInfo.labelInfos) : []' :coinList="coinList">
							...
							<view v-if="pageType == 2">
								...
								<GoodsTree v-else :pageType="pageType"
									:labelInfos='goodsInfo.purchaseOrderItemVOS[0].labelInfos ? goodsInfo.purchaseOrderItemVOS[0].labelInfos : []'
									:coinList="coinList">
									<template v-slot:markUnitPrice>{{ goodsInfo.purchaseOrderItemVOS[0].transactionUnitPrice }}</template>
								</GoodsTree>
							</view>
						</view>
						<view class="pay" v-if="goodsInfo.orderStatus == 3">
							<button style="background-color: #ABABAB;cursor: not-allowed;" hover-class="none">已售謦</button>
						</view>
						...
						<view class="pay" v-else>
							<button v-if="pageType == 1 && !orderCode && !goodsInfo.currentCargoStatus" @click="onBuy">立即购买</button>
							<button class="preorder-but" v-if="pageType == 1 && !orderCode && goodsInfo.currentCargoStatus"
								@click="onOpenFut">
								<view>立即预购</view>
								<view>
									<view>支付保证金</view>
						...
						<view class="submit-info" @click="goDownApp(-1)">
							<view class="left">
								<view class="link">
		...
		<YppFooter></YppFooter>
		<popup-futures v-if="is_futures" :type="0" @close="onFuturesclose" @suer="onFuturesSuer"></popup-futures>
	</view>
</template>
...
<script setup>
	...
	function getDetails(id) {
		if (pageType.value == 1) {
			details(id).then(res => {
				...
				imgList.value = res.data.pictureDetails == '' ? res.data.firstItemImageUrl.split() : res.data
					.pictureDetails.split(',')
				...
				RightsList.value[0].name = (!res.data.appraisalInstitutionId || res.data.appraisalInstitutionId *
					1 < 1000) ? '无评级' : '评级币'
		...
		if (pageType.value == 2) {
			detailsBuy(id).then(res => {
				...
				if (res.data.purchaseOrderItemVOS[0].specItemList.length !== 0) {
					...
					imgList.value = [
						'https://ypp-china-1325306284.cos.ap-shanghai.myqcloud.com/yph/common/money_big_image/long-b.jpg',
						'https://ypp-china-1325306284.cos.ap-shanghai.myqcloud.com/yph/common/money_big_image/she-b.jpg'
					]
				}
				RightsList.value[0].name = (!res.data.purchaseOrderItemVOS[0].appraisalInstitutionId || res.data
					.purchaseOrderItemVOS[0].appraisalInstitutionId * 1 < 1000) ? '无评级' : '评级币'
				RightsList.value[1].name = res.data.needTradeMargin ? '保证金' : '无保证金'
				RightsList.value[2].name = res.data.transactionMode ? '担保交易' : '非担保交易'
	...
	const onBuy = () => {
		...
		if (token) {
			uni.navigateTo({
				url: `/pages/buy/buy?id=${id.value}&nickName=${marchantInfo.value.nickName}&businessType=${marchantInfo.value.businessType}`
			})
		} else {
			uni.showToast({
				title: '请先登录',
	...
	const onOpenFut = () => {
		if (uni.getStorageSync('is_preorder')) {
			onBuy()
			return
		}
		ProhibitScroll()
